<?php
/*
Template Name: ACP Admin Screenings
*/
...
if (empty($_REQUEST['dctrl']) || !is_array($_REQUEST['dctrl'])) {
	$ctrl = array(
		...
		'datetype' => '',
		'orderarch' => 0,
		'scrtype' => (empty($_REQUEST['scrtype']) ? 'any' : $_REQUEST['scrtype']),
...
if (!empty($ctrl['action']) && !empty($ctrl['ids'])) {
	...
	switch ($ctrl['action']) {
		case 'do_archive':
			$_SESSION['admin_saved_msg'] = "{$num} screening{$s} archived";
			ACPManager::reg_archive($ctrl['ids']);
			break;
		case 'do_unarchive':
			$_SESSION['admin_saved_msg'] = "{$num} screening{$s} unarchived";
			ACPManager::reg_archive($ctrl['ids'], 0);
			break;
		case 'do_delete':
			$_SESSION['admin_saved_msg'] = "{$num} screening{$s} deleted";
			ACPManager::screening_multi_delete($ctrl['ids']);
			break;
		default : break;
	}
}
...
switch($scrtype) {
	case 'bmi':
		$scrtype = 'ScreeningBMI';
		break;
	case 'bp':
		$scrtype = 'ScreeningBP';
		break;
	case 'vision':
		$scrtype = 'ScreeningVision';
		break;
	case 'vitals':
		$scrtype = 'ScreeningVitals';
		break;
	case 'any':
	default:
		$scrtype = 'Screen%';
...
$total_found = 0;
if ($datetype == 'on') {
	$regs = ACPManager::list_screenings_on_day($event_id, $scrtype, $date, $archived, $amount, $start, $orderby, $orderdir, $ordercat, $total_found);
}
else {
	$start_date = ($datetype == 'ona' ? $date : null);
	$end_date = ($datetype == 'onb' ? $date : null);
	$regs = ACPManager::list_screenings($event_id, $scrtype, $start_date, $end_date, $archived, $amount, $start, $orderby, $orderdir, $ordercat, $total_found);
}
...
<div id="main">
	...
	<form action="<?php echo get_page_link(9); ?>" method="post" id="frm_controls">
	...
	<input type="hidden" name="dctrl[scrtype]" />
	</form>
	<div class="container">
		
		<div class="data-group" style="max-width: 1200px;">
		<!-- begin controls -->
		<div class="data-control-group">
			...
			<div class="data-control">
				...
				<option value="type" <?php if ($ctrl['orderby'] == 'type') { echo 'selected="selected"'; } ?>>Type</option>
				</select>
				...
				<select class="ctrl_scrtype">
				<option value="">Screen Type</option>
				<option value="bp" <?php if ($ctrl['scrtype'] == 'bp') { echo 'selected="selected"'; } ?>>BP</option>
				<option value="vitals" <?php if ($ctrl['scrtype'] == 'vitals') { echo 'selected="selected"'; } ?>>Vitals</option>
				<option value="bmi" <?php if ($ctrl['scrtype'] == 'bmi') { echo 'selected="selected"'; } ?>>BMI</option>
				<option value="vision" <?php if ($ctrl['scrtype'] == 'vision') { echo 'selected="selected"'; } ?>>Vision</option>
				</select>
		...
		<tr class="header">
			...
			<td style="width: 30%;">
			<a class="sortable" href="<?php echo get_page_link(9) . "?orderby=event&orderdir={$event_dir}"; ?>">Event</a>
			</td>
			<td style="width: 10%;">
			<a class="sortable" href="<?php echo get_page_link(9) . "?orderby=type&orderdir={$type_dir}"; ?>">Type</a>
			</td>
			<td style="width: 5%;">Archived</td>
			<td style="width: 20%;">
			<a class="sortable" href="<?php echo get_page_link(9) . "?orderby=created&orderdir={$created_dir}"; ?>">Date</a>
			</td>
		</tr>
		<?php if (empty($regs)): ?>
		<tr>
			<td colspan="6" class="txtcenter">No screenings found</td>
		</tr>
		<?php else: ?>
...
jQuery(document).ready(function() {
	...
	jQuery('button.btn_do_order, button.btn_results, button.btn_page').click(function(e) {
		...
		frm.find('input[name=dctrl\\[scrtype\\]]').val(grp.find('.ctrl_scrtype').val());
		if (cats.length) {
