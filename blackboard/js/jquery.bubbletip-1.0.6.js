...
; (function($) {
	...
	$.fn.extend({
		bubbletip: function(tip, options) {
			...
			if ($('table.bubbletip #' + $(tip).eq(0).attr('id')).length > 0) {
				//return this;
				$('table.bubbletip #' + $(tip).eq(0).attr('id')).remove();
			}
			...
			if (!_this.attr('data-bubbletip_tips')) {
				_this.attr('data-bubbletip_tips', [[_tip.eq(0).attr('id'), _bindIndex]]);
			} else {
				_this.attr('data-bubbletip_tips', $.merge(_this.attr('data-bubbletip_tips'), [[_tip.eq(0).attr('id'), _bindIndex]]));
			}
			...
			$(_this).bind(_calc.bindShow, function() {
				if (_timeoutAnimate) {
		...
		removeBubbletip: function(tips) {
			var tipsActive = new Array();
			var tipsToRemove = new Array();
			var tipsActiveAdjusted = new Array();
			var arr, i, ix, tmp;
			var elem;

			//tipsActive = $.makeArray($(this).attr('data-bubbletip_tips'));
			tmp = $(this);
			if (!tmp || !tmp.attr || !tmp.attr('data-bubbletip_tips')) {
				return;
			}
			tmp = tmp.attr('data-bubbletip_tips').split(',');
			for (i = 0; i < tmp.length; i+=2) {
				tipsActive.push([tmp[i], tmp[i+1]]);
			}

			// convert the parameter array of tip id's or elements to id's
			arr = $.makeArray(tips);
			for (i = 0; i < arr.length; i++) {
				tipsToRemove.push($(arr[i]).eq(0).attr('id'));
			}
			...
			for (i = 0; i < tipsActive.length; i++) {
				...
				if ((tipsToRemove.length == 0) || ((ix = $.inArray(tipsActive[i][0], tipsToRemove)) >= 0)) {
					...
					elem = $('#' + tipsActive[i][0]).closest('table');
					/*while (elem.prop('tagName').toLowerCase() != 'table') {
						elem = elem.parent();
					}*/
			...
			$(this).attr('data-bubbletip_tips', tipsActiveAdjusted);
